 ...
 * they move.
 *
 * This is no longer the only price list. An administrator adds and reprices
 * models from /admin/models, and those rows win — see
 * convex/lib/modelCatalogue.ts. This table stays as the floor: it is what a
 * fresh deployment knows, and what history written before that table existed
 * is costed by. Anything added here should still be added here, so it survives
 * an empty database and goes through review.
 */
 ...
 *
 * `override` is the `aiModels` row an administrator added or repriced from the
 * dashboard. It wins over the built-in table so a price can move, or a model
 * this file has never heard of can be costed, without a deploy — see
 * `lookupPrice` in convex/models.ts, which is what every caller passes.
 */
export function costNanoUsd(
  ...
  const price = override ?? MODEL_PRICES[model];
